...
const ResumeRewriteCard: React.FC<ResumeRewriteCardProps> = ({ onRewrite }) => {
  ...
  const handleRewrite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/resume-rewrite`, {
        method: "POST",
      ...
      if (typeof window !== 'undefined' && window.plausible) {
        window.plausible.trackEvent('Resume Rewrite');
      }
    } catch (err: any) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <h3 className="text-lg font-bold mb-2 text-blue-700">Rewrite Your Resume</h3>
        <form onSubmit={handleRewrite} className="flex flex-col gap-3">
