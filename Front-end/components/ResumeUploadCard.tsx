...
const ResumeUploadCard: React.FC<ResumeUploadCardProps> = ({ onAnalysis }) => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const analysisRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/resume/parsed`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      ...
      if (typeof window !== 'undefined' && window.plausible) {
        window.plausible.trackEvent('Resume Upload', { props: { fileType: selectedFile?.type } });
      }
    } catch (err: any) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <h3 className="text-lg font-bold mb-2 text-blue-700">Upload Your Resume</h3>
