...
export default function PaymentStatusPage() {
  ...
  const [status, setStatus] = useState<'loading' | 'success' | 'failed' | 'pending' | 'unknown'>(
    'loading'
  );
    ...
    <div className="max-w-lg mx-auto p-8">
      ...
      {paymentId && (
        <>
          ...
          {status === 'pending' && (
            <div className="text-yellow-600">Payment is pending. Please wait...</div>
          )}
          {status === 'success' && (
          ...
          {details && (
            <pre className="bg-gray-100 p-2 mt-4 rounded text-xs overflow-x-auto max-h-64">
